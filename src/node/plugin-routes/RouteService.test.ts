...
describe('RouteService', async () => {
  ...
  test('conventional route by file structure', () => {
    const routeData = routeService.getRouteData().map((item) => ({
      ...item,
      fileAbsolutePath: item.fileAbsolutePath.replace(testDir, 'TEST_DIR'),
    }));
    expect(routeData).toMatchInlineSnapshot(`
      [
        {
          "fileAbsolutePath": "TEST_DIR/a.mdx",
          "routePath": "/a",
        },
        {
          "fileAbsolutePath": "TEST_DIR/guide/index.mdx",
          "routePath": "/guide/",
        },
      ]
    `);
